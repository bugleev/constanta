...
test("Employeelist renders correct number of items", () => {
...
test("EditMenu renders correct number of items", () => {
  const func = jest.fn;
...
test("adds item on 'add' button click", () => {
  const { container, getByText } = render(<App />);
...
test("removes item from the list on 'remove' button click", () => {
